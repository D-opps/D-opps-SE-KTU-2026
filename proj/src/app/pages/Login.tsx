...
import { useNavigate, Link } from 'react-router'; // Тільки react-router
...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/token/', {
        email: email, // або username, залежить від налаштувань бекенду
        password: password,
      });
      ...
      if (response.status === 200) {
        ...
        // --- НОВИЙ БЛОК: Отримуємо дані про себе ---
        try {
          const userResponse = await axios.get('http://127.0.0.1:8000/api/users/me/', {
            headers: { 'Authorization': `Bearer ${access}` }
          });
          
          // Зберігаємо той самий ID, який бекенд пише в seller_id
          localStorage.setItem('userId', userResponse.data.id); 
          localStorage.setItem('userName', userResponse.data.full_name || email.split('@')[0]);
        } catch (userErr) {
          console.error("Could not fetch user profile, using fallback");
          // Якщо ендпоінта /me/ немає, спробуй дістати ID з токена (декодувати) 
          // або переконайся, що бекенд його надсилає
        }
        // -------------------------------------------
  ...
  // Google логіка залишається без змін, тільки перевір шлях на бекенді
