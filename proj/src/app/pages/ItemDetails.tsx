...
import { ArrowLeft, Tag as TagIcon, MessageCircle as ChatIcon, Loader2 as LoaderIcon, AlertCircle, Flag } from 'lucide-react';
...
export function ItemDetails() {
  ...
  const handleReport = () => {
    const token = localStorage.getItem('accessToken');
    if (!token) {
      toast.error("Будь ласка, увійдіть, щоб подати скаргу");
      navigate('/login');
      return;
    }
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className={`px-3 py-1 text-xs font-bold uppercase tracking-widest rounded-full ${
                item.is_used ? 'bg-orange-50 text-orange-600' : 'bg-green-50 text-green-600'
              }`}>
                {item.is_used ? 'Used' : 'New'}
              </span>
            </div>
          ...
          <div className="pt-6 border-t border-gray-100">
            <div className="flex items-center gap-4 mb-8">
              <div className="w-14 h-14 bg-blue-600 rounded-2xl flex items-center justify-center font-bold text-white text-xl uppercase shadow-md">
            ...
            {isOwner ? (
              <div className="p-4 bg-gray-50 rounded-2xl text-gray-500 italic flex items-center gap-3 border border-dashed border-gray-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button 
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-4 bg-gray-900 text-white rounded-2xl hover:bg-black font-bold shadow-lg transition-all active:scale-95 hover:shadow-xl"
                >
                  <ChatIcon size={20} /> Contact Seller
                </button>
                <button 
                  onClick={handleProposeExchange} 
                  className="flex items-center justify-center gap-2 px-6 py-4 bg-white text-gray-900 border-2 border-gray-900 rounded-2xl hover:bg-gray-50 font-bold transition-all active:scale-95"
                >
